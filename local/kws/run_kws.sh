...
exp=exp_baseline/tri3_7000_140000
...
if [ $stage -le 2 ] ; then
  ...
  #steps/align_fmllr.sh --nj 30 --cmd "$cmd" \
  #  $data data/lang $exp exp_baseline/tri3_7000_140000_new

  local/kws/create_hitlist.sh $data $lang data/local/lang \
    exp_baseline/tri3_7000_140000_new $output
fi
...
#system=exp_baseline/chain/tdnn1g_sp_online/decode_dev_IITM_new/
#system=exp_baseline/kws_mono/decode/
#system=exp_baseline/kws_tdnn/decode_final/
system=exp_baseline/chain/tdnn1g_sp/decode_dev_split/
if [ $stage -le 4 ]; then
